...
function assertFirebase(){
  if (typeof db === "undefined") {
    console.error("Firestore (db) no está inicializado. Revisa que cargues assets/js/firebase-init.js antes de app.js");
    return false;
  }
  return true;
}

async async function cargarPrendas() {
  if (!assertFirebase()) return;
  try {
...
function actualizarInterfaz() {
  ...
  if (totalEl) totalEl.innerText = `Total: ${formatearSoles(total)}`;
