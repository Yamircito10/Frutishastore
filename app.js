...
// ==========================================
// 1. LÓGICA DE LA TIENDA Y FILTROS
// ==========================================
...
function generarPDFRecibo(productos, totalVenta, metodoPago) {
  ...
  doc.setTextColor(216, 27, 96); doc.text("977 757 369", 42, contactoY + 7, { align: "center" });
...
async function enviarWhatsApp() {
  ...
  if(numeroCliente && nombreCliente) {
     ...
     } catch(e) { console.error("No se pudo guardar el cliente"); }
  }
...
window.navegarSPA = function(idDestino) {
  ...
  if (idDestino === 'vista-inventario') {
      cargarInventarioSPA();
      cargarHistorialMovimientosSPA(); 
  }
};
  ...
  try {
    ...
    html += `<h3>🏦 Cuadre de Caja (Total)</h3>
             <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 20px; text-align: center;">
                <div style="background: white; padding: 15px; border-radius: 10px; border-top: 4px solid #f1c40f; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                    <div style="font-size: 20px;">💵</div><div style="font-size: 11px; color: #888;">EFECTIVO</div>
                    <div style="font-size: 14px; font-weight: bold; color: #2ecc71;">${formatearSoles(resumenPagos["Efectivo"])}</div>
                </div>
                <div style="background: white; padding: 15px; border-radius: 10px; border-top: 4px solid #9b59b6; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                    <div style="font-size: 20px;">📱</div><div style="font-size: 11px; color: #888;">YAPE/PLIN</div>
                    <div style="font-size: 14px; font-weight: bold; color: #2ecc71;">${formatearSoles(resumenPagos["Yape/Plin"])}</div>
                </div>
                <div style="background: white; padding: 15px; border-radius: 10px; border-top: 4px solid #3498db; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                    <div style="font-size: 20px;">💳</div><div style="font-size: 11px; color: #888;">TARJETA</div>
...
async function cargarClientesSPA() {
  ...
  try {
    ...
    snap.forEach(doc => {
      let c = doc.data();
      html += `<li style="background: var(--tarjetas); padding:15px; margin-bottom:10px; border-radius:10px; border-left:5px solid #9b59b6; display:flex; justify-content:space-between; align-items:center; box-shadow: 0 4px 6px rgba(0,0,0,0.05); color: var(--texto);">
                <div>
                  <strong style="font-size: 16px;">👤 ${c.nombre}</strong><br>
                  <small style="color:#888;">Última compra: ${c.ultimaCompra}</small>
                </div>
                <a href="https://wa.me/51${c.celular}" target="_blank" style="background:#25D366; color:white; padding:8px 12px; border-radius:20px; text-decoration:none; font-weight:bold; font-size:12px;">📱 Escribir</a>
              </li>`;
    });
    div.innerHTML = html + `</ul>`;
...
async function cargarGastosSPA() {
  ...
  try {
    ...
    snap.forEach(doc => {
      const g = doc.data();
      html += `<li style="background: var(--tarjetas); padding:15px; margin-bottom:10px; border-radius:10px; border-left:5px solid #e74c3c; display:flex; justify-content:space-between; align-items:center; box-shadow: 0 4px 6px rgba(0,0,0,0.05); color: var(--texto);">
                <div>
                  <strong style="font-size: 15px;">${g.motivo}</strong><br>
                  <small style="color:#888;">📅 ${g.fechaTexto} - 🕒 ${g.hora} | 🏷️ ${g.categoria}</small>
                </div>
                <span style="font-weight:bold; color:#e74c3c; font-size: 16px;">- ${formatearSoles(g.monto)}</span>
              </li>`;
    });
    div.innerHTML = html + "</ul>";
  } catch (e) { div.innerHTML = "<p>Error cargando los gastos.</p>"; }
}

// ==========================================
// 🕵️‍♂️ HISTORIAL DE MOVIMIENTOS DE ALMACÉN
// ==========================================
...
async function cargarHistorialMovimientosSPA() {
  ...
  try {
    ...
    div.innerHTML = snap.docs.map(doc => {
      ...
      return `<div style="font-size:13px; padding:10px; border-bottom:1px solid var(--borde); display:flex; justify-content:space-between; align-items:center; color:var(--texto);">
                <div>
                  <strong>${m.fechaTexto} - ${m.hora}</strong><br>
                  <span>👕 ${m.prendaNombre} (Talla: ${m.talla})</span><br>
                  <small style="color:#888;">👤 Por: ${m.usuario}</small>
                </div>
                <span style="color:${color}; font-weight:bold; font-size: 16px;">${signo}${m.cantidad}</span>
              </div>`;
    }).join("");
  } catch (e) {}
}
